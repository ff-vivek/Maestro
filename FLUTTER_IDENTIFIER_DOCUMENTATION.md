...
Flutter's `Semantics(identifier: ...)` property creates a stable `flt-semantics-identifier` HTML attribute that Maestro can use via the `flutterId` selector.

---

## The Three Key Concepts

### 1. `flutterId` (Maestro Selector)
...
**Naming Convention:** 
- **YAML:** `flutterId` (kebab-case with hyphen)
...
──────────────────────              ──────────────────
attributes['flutterId']     ───►   flutterIdMatches(flutterId)
  = 'submit_button'                   filters by flutterId
                                             │
        ┌────────────────────────────────────┘
        │
        ▼
...
parseDomAsTreeNodes()               YamlElementSelector
  attributes["flutterId"]    ───►    @JsonProperty("flutterId")
...
2. If found, stores it as `flutterId` in the element attributes
...
2. Filters all nodes to find those with matching `flutterId` attribute
...
This ensures the `flutterId` attribute is properly passed through the view hierarchy.

**Note:** Maestro has two web driver implementations that both support flutterId:
...
Both parse the `flutterId` attribute identically.
...
data class YamlElementSelector(
    ...
    val id: String? = null,
    @JsonProperty("flutterId") val flutterId: String? = null,  // ← Defined here
    val width: Int? = null,
    val height: Int? = null,
    // ... other selectors
)
...
**Key Points:**
- Uses `@JsonProperty("flutterId")` annotation to map kebab-case YAML to camelCase Kotlin
- The Kotlin property is named `flutterId` (camelCase)
- The YAML selector is `flutterId` (kebab-case with hyphen)
...
data class ElementSelector(
    ...
    fun description(): String {
        ...
        flutterId?.let {
            descriptions.add("flutterId: $it")  // ← Displayed in logs
        }
...
- The `description()` method outputs it as `flutterId` for user-facing logs
- This model is used throughout Maestro's orchestration layer

#### Step 8: Test Uses flutterId Selector
...
1. YAML parser reads `flutterId: "submit_button"` from test file
...
5. Finds the element with matching `flutterId` attribute
...
**Key Points:**
- Extracts `flt-semantics-identifier` as a **separate** attribute called `flutterId`
...
```kotlin
// When Maestro encounters flutterId selector
...
Maestro has **two web driver implementations**, both supporting flutterId identically:
...
**Key Points:**
- Both drivers parse `flutterId` from JavaScript-extracted attributes identically
...
# Tap to check
- tapOn:
    flutterId: "remember_me_checkbox"
...
### Example 5: Combining flutterId with Other Selectors
...
- assertVisible:
    flutterId: "submit_form_button"
    enabled: false

# Fill out the form
- tapOn:
    flutterId: "name_field"
- inputText: "John Doe"
...
| Kotlin Filter | `maestro-client/src/main/java/maestro/Filters.kt` | Filters elements by `flutterId` |
| WebDriver Parser (Selenium) | `maestro-client/src/main/java/maestro/drivers/WebDriver.kt` | Parses and passes `flutterId` attribute |
| CdpWebDriver Parser (CDP) | `maestro-client/src/main/java/maestro/drivers/CdpWebDriver.kt` | Parses and passes `flutterId` attribute (CDP-based) |
| YAML Selector Definition | `maestro-orchestra/src/main/java/maestro/orchestra/yaml/YamlElementSelector.kt` | Defines `flutterId` YAML selector |
...
**Note:** Both `WebDriver` (Selenium) and `CdpWebDriver` (Chrome DevTools Protocol) implementations parse flutterId identically.
...
**Why:**
- `identifier`: Provides stable `flutterId` selector
...
### Issue: flutterId selector not finding element
...
### Issue: Multiple elements with same flutterId
...
### Issue: Element has flutterId but tap fails
...
| **Maestro Selector** | YAML test | `flutterId: "my_id"` | Select element |
...
3. **maestro-web.js** extracts it as `attributes['flutterId'] = 'submit_button'`
4. **Filters.kt** provides `flutterIdMatches("submit_button")` filter function
5. **WebDriver/CdpWebDriver** parses DOM and preserves `flutterId` attribute
6. **YamlElementSelector** defines `@JsonProperty("flutterId")` mapping
7. **ElementSelector** model stores `flutterId` property
8. **Maestro Test** uses `flutterId: "submit_button"` selector → **Element Found** ✅
