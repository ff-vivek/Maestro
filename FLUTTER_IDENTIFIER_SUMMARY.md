...
### 1. `flutterId` (Maestro Test Selector)

**Used in:** Maestro YAML test files  
**Purpose:** Select elements by their Flutter identifier  
**Naming:** Kebab-case (`flutterId`) in YAML, camelCase (`flutterId`) in Kotlin  
**Example:**
```yaml
- tapOn:
...
Semantics(               →   <flt-semantics          →   - tapOn:
  identifier:                  flt-semantics-              flutterId:
...
```
**Purpose:** Extracts `flt-semantics-identifier` HTML attribute and stores as `flutterId`
...
fun flutterIdMatches(flutterId: String): ElementFilter {
    return { nodes ->
        nodes.filter { it.attributes["flutterId"] == flutterId }
    }
}
```
**Purpose:** Filters elements by matching `flutterId` attribute
...
```
**Purpose:** Both drivers parse `flutterId` from JavaScript and preserve in TreeNode
...
```
**Purpose:** Maps YAML `flutterId` (kebab-case) to Kotlin `flutterId` (camelCase)
...
data class ElementSelector(
    ...
    fun description(): String {
        flutterId?.let { descriptions.add("flutterId: $it") }
    }
}
```
**Purpose:** Stores flutterId value and provides user-facing descriptions
...
Semantics(
  identifier: 'submit_registration_form',  // For flutterId selector
...
- tapOn:
    flutterId: "submit_registration_form"  # Stable ID
...
- tapOn:
    flutterId: "submit_registration_form"
    text: "Submit"
    enabled: true
```
