...
You can now use the `flutterId` selector for Flutter identifiers. First, configure the identifier mapping in your workspace `config.yaml`:
...
Then use the `flutterId` selector in your flows:
...
Maestro extracts `flt-semantics-identifier` as a separate attribute called `flutterId`. You can then use the `flutterId` selector to target elements by their Flutter identifier:

- `flutterId` selector → matches `flt-semantics-identifier` attribute (Flutter's stable identifier)
...
# The workspace config.yaml will be automatically detected
maestro test e2e/workspaces/flutter_web_identifier/test_flutter_identifier.yaml
```

Note: The `identifierConfig` is now configured at the workspace level in `config.yaml` under `platform.web.identifierConfig`. This makes it clearer that this is a web-specific configuration and applies to all flows in the workspace.
...
- ✅ Using `flutterId` for text fields
- ✅ Using `flutterId` for buttons
- ✅ Using `flutterId` for checkboxes
- ✅ Using `flutterId` for tabs with state checks
- ✅ Combining `flutterId` with other selectors (like `enabled`, `selected`)

## Implementation Details

The feature works by:
1. Modifying `maestro-web.js` to extract the `flt-semantics-identifier` attribute as `flutterId`
2. Adding a new `flutterId` selector in Maestro's element selection system
3. The `flutterId` selector specifically targets the `flutterId` attribute
